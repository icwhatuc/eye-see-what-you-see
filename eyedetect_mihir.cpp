...
int main(int argc, char *argv[])
{
    ...
    Mat prevframe_mat(CAMWIDTH, CAMHEIGHT, CV_8UC3), matdiff, prevframe_gray(CAMWIDTH, CAMHEIGHT, CV_8UC1), currframe_mat(CAMWIDTH, CAMHEIGHT, CV_8UC3), currframe_matcopy, 
		currframe_gray(CAMWIDTH, CAMHEIGHT, CV_8UC1), diff_img, diff_copy, colored_diff_img, element, 
	...
	*/
	...
	{	
		...
		//std::cout << arduino_state << std::endl;
		//arduino.flush();

		capture >> prevframe_mat;  //prevframe contains the dark image - or it should
		...
		capture >> currframe_mat;  //currframe contains the bright image - or it should
		currframe_matcopy = currframe_mat.clone();
		cvtColor(currframe_mat.clone(), currframe_gray, CV_BGR2GRAY);
		
		sprintf(filename, "imgs/%da_previmg.jpg", framecount);
		imwrite(filename, prevframe_gray);
	
		sprintf(filename, "imgs/%db_currimg.jpg", framecount);
		imwrite(filename, currframe_gray);
		...
		sprintf(filename, "imgs/%ddiffimg.jpg", framecount);
		imwrite(filename, diff_img);
		...
		putText(currframe_mat, num_eyes_str, Point(30,100), FONT_HERSHEY_COMPLEX_SMALL, 3, RED, 1, CV_AA);
