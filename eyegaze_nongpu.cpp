...
inline unsigned char calcThreshold(Mat &img);
Rect haarClassify(CascadeClassifier &cascade, Mat &image, int w, int h);
inline Mat getEyePairImage(Mat &mat_image, Rect &r);
...
int main(int argc, char *argv[])
{
	...
	while(1)
	{
		...
		cvtColor(im1, edcs.grayframe1_m, CV_BGR2GRAY);

		capture >> im2;
		im2 = im2.clone();
		flip(im2, im2, 1);
		
		cvtColor(im2, edcs.grayframe2_m, CV_BGR2GRAY);
		edcs.currColorFrame = im2.clone();
		...
		*/
		...
		if (edcs.knownpairs.size() > 0 && edcs.knownpairs[0].isCalibrated == true)
			cout << randomPoint.x << ' ' << randomPoint.y << ' ';
		// END DEBUG----------------------------------------
...
void checkKnownPairs(eyedetectcomponents &edcs)
{
	...
	for(pair = 0; pair < edcs.knownpairs.size(); pair++) {
		for(eye = 0; eye < 2; eye++) {
			...
			diffimg_m, thresholdedimg_m;
			grayregion1 = grayregion1.clone();
			grayregion2 = grayregion2.clone();
			// get the difference image
			absdiff(grayregion1, grayregion2, diffimg_m);
			blur(diffimg_m, diffimg_m, Size(3, 3), Point(-1,-1));
			threshold(diffimg_m, thresholdedimg_m, edcs.threshold*3/4, 255, CV_THRESH_BINARY);
			...
			*/
			...
			Point centroidLoc = getCentroidLoc(thresholdedimg_m);

			// If thresholded diff has no bright pixels, use minMaxLoc of non-thresholded diff
			if (centroidLoc.x == -1)
				minMaxLoc(diffimg_m,0,0,0,&centroidLoc);
			...
			Rect eye_rect = haarEyeClassify(edcs.eye_cascade, r2);
...
inline unsigned char calcThreshold(Mat &img_m)
{
	Mat hist_m;
	...
	calcHist(&img_m, 1, 0, Mat(), hist_m, 1, &bins, &histRange, true, false);
	
	for (threshbin = HIST_BINS-1; threshbin > 0; threshbin--) {
		histcount += hist_m.at<float>(threshbin);
...
void lookForNewEyes(eyedetectcomponents &edcs)
{
	Mat grayframe1_g, grayframe2_g, diffimg_g, threshimg_m;

	vector<KeyPoint> eyeCandidates;
	vector<Point> detectedEyes;

	grayframe1_g = edcs.grayframe1_m.clone();
	grayframe2_g = edcs.grayframe2_m.clone();

	// Obtain difference image and blur
	absdiff(grayframe2_g, grayframe1_g, diffimg_g);
	blur(diffimg_g, diffimg_g, Size(3, 3), Point(-1,-1));

	// Thresholding - find threshold based on histogram
	unsigned char thresh = calcThreshold(diffimg_g);
	edcs.threshold = thresh;
	// DEBUG: For display purposes only
	threshold(diffimg_g, threshimg_m, thresh, 255, CV_THRESH_BINARY);
	...
	edcs.params.minThreshold = thresh-1;
	edcs.params.maxThreshold = thresh+1;
	Ptr<FeatureDetector> blob_detector = new SimpleBlobDetector(edcs.params);
	blob_detector->create("SimpleBlob");
	blob_detector->detect(diffimg_g, eyeCandidates);
	imshow(W_DIFF, diffimg_g);
	...
	for (int c = 0; c < eyeCandidates.size(); c++) {
		...
		Mat candidateimg_m = edcs.grayframe2_m(candidate_region); candidateimg_m = candidateimg_m.clone();

		// check against haar classifier
		Rect eye_rect = haarEyeClassify(edcs.eye_cascade, candidateimg_m);
		bool haar_detected = eye_rect.area() > 1;
	...
	for (int eye1 = 0; eye1 < detectedEyes.size(); eye1++) {
		for (int eye2 = eye1+1; eye2 < detectedEyes.size(); eye2++) {
			...
			Mat eyepair_g = getEyePairImage(edcs.grayframe2_m, eyepair_rect_in);
	...
	for(int pair = 0; pair < edcs.knownpairs.size(); pair++)
	{
		...
		if (edcs.knownpairs[pair].isCalibrated)
		{
			...
			//DEBUG: try lowpass----------------------
			const float lowpassWeight= 0.7; // How much the old data is considered
			...
			// Output for piped poster display
...
Rect haarClassify(CascadeClassifier &cascade, Mat &image, int w, int h) 
{
	...
	cascade.detectMultiScale(eqImage, features);
